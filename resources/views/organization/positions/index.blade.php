<x-app-layout>
    <x-slot name="header">
        <div class="flex justify-between items-center">
            ...
            <div>
                <a href="{{ route('positions.create') }}" class="inline-flex items-center px-4 py-2 text-sm font-medium text-white bg-green-600 border border-transparent rounded-md shadow-sm hover:bg-green-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-green-500">
                ...
                <div class="p-6 text-gray-900 dark:text-gray-100">
                    ...
                    <form method="GET" action="{{ route('positions.index') }}">
                        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                            ...
                            <div>
                                <label for="name" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">ชื่อตำแหน่ง</label>
                ...
                <div class="p-6 text-gray-900 dark:text-gray-100">
                    <div class="overflow-x-auto">
                        <div class="flex justify-between items-center mb-4">
                            <h3 class="text-lg font-semibold">รายการตำแหน่ง</h3>
                        ...
                        <table class="min-w-full bg-white dark:bg-gray-700 border border-gray-300 dark:border-gray-600">
                            <thead>
                                <tr class="bg-gray-100 dark:bg-gray-600 text-gray-700 dark:text-gray-200">
                                    ...
                                    <th class="py-2 px-4 border-b border-gray-300 dark:border-gray-600 text-left">ชื่อตำแหน่ง</th>
                                    <th class="py-2 px-4 border-b border-gray-300 dark:border-gray-600 text-left">รหัสตำแหน่ง</th>
                                    <th class="py-2 px-4 border-b border-gray-300 dark:border-gray-600 text-left">ระดับ</th>
                            ...
                            <tbody>
                                ...
                                <tr class="hover:bg-gray-200 dark:hover:bg-gray-600 border-b border-gray-200 dark:border-gray-700">
                                    <td class="py-2 px-4">{{ $position->id }}</td>
                                    <td class="py-2 px-4">{{ $position->company ? $position->company->name : '-' }}</td>
                                    <td class="py-2 px-4">{{ $position->department ? $position->department->name : '-' }}</td>
                                    <td class="py-2 px-4">{{ $position->name }}</td>
                                    <td class="py-2 px-4">{{ $position->code ?: '-' }}</td>
                                    <td class="py-2 px-4">{{ $position->level ?: '-' }}</td>
                                    <td class="py-2 px-4">
                                    ...
                                    <td class="py-2 px-4 text-center">
                                        <div class="flex justify-center space-x-2">
                                            <a href="{{ route('positions.show', $position->id) }}" class="text-blue-500 hover:text-blue-700">
                                                <svg class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z" />
                                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z" />
                                                </svg>
                                            </a>
                                            <a href="{{ route('positions.edit', $position->id) }}" class="text-yellow-500 hover:text-yellow-700">
                                                <svg class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" />
                                                </svg>
                                            </a>
                                            <form action="{{ route('positions.destroy', $position->id) }}" method="POST" class="inline" onsubmit="return confirm('คุณแน่ใจหรือไม่ที่จะลบตำแหน่งนี้?');">
                                                @csrf
                                                @method('DELETE')
                                ...
                                @empty
                                <tr>
                                    <td colspan="8" class="py-4 text-center text-gray-500 dark:text-gray-400">ไม่พบข้อมูลตำแหน่ง</td>
                                </tr>
                                @endforelse
                            </tbody>
                        </table>
