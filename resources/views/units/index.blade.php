<x-app-layout>
    <x-slot name="header">
        <div class="flex justify-between items-center">
            ...
            <div>
                <a href="{{ route('units.create') }}" class="inline-flex items-center px-4 py-2 text-sm font-medium text-white bg-green-600 border border-transparent rounded-md shadow-sm hover:bg-green-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-green-500">
                    <svg class="-ml-1 mr-2 h-5 w-5" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4v16m8-8H4" />
                    </svg>
                    {{ __('เพิ่มหน่วยนับใหม่') }}
                </a>
            </div>
        </div>
    </x-slot>
                ...
                <div class="p-6 text-gray-900 dark:text-gray-100">
                    ...
                    <form method="GET" action="{{ route('units.index') }}">
                        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                            <div>
                                ...
                                <input type="text" name="search" id="search" value="{{ request('search') }}"
                                    ...
                                    placeholder="ชื่อ, รหัส, สัญลักษณ์">
                            </div>
                            <div>
                        ...
                        <div class="mt-4 flex justify-end">
                            <a href="{{ route('units.index') }}" class="px-4 py-2 text-sm font-medium text-gray-700 bg-white border border-gray-300 rounded-md shadow-sm hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500 mr-2 dark:bg-gray-700 dark:text-white dark:border-gray-600 dark:hover:bg-gray-600">
                                รีเซ็ต
                            </a>
                ...
                <div class="p-6 text-gray-900 dark:text-gray-100">
                    <div class="overflow-x-auto">
                        <div class="flex justify-between items-center mb-4">
                            <h3 class="text-lg font-semibold">รายการหน่วยนับ</h3>
                        ...
                        <table class="min-w-full bg-white dark:bg-gray-700 border border-gray-300 dark:border-gray-600">
                            <thead>
                                <tr class="bg-gray-100 dark:bg-gray-600 text-gray-700 dark:text-gray-200">
                                    <th class="py-2 px-4 border-b border-gray-300 dark:border-gray-600 text-center">No.</th>
                                    <th class="py-2 px-4 border-b border-gray-300 dark:border-gray-600 text-left">ชื่อ</th>
                                    <th class="py-2 px-4 border-b border-gray-300 dark:border-gray-600 text-left">รหัส</th>
                                    <th class="py-2 px-4 border-b border-gray-300 dark:border-gray-600 text-left">สัญลักษณ์</th>
                                    <th class="py-2 px-4 border-b border-gray-300 dark:border-gray-600 text-left">บริษัท</th>
                                    <th class="py-2 px-4 border-b border-gray-300 dark:border-gray-600 text-left">หน่วยฐาน</th>
                                    <th class="py-2 px-4 border-b border-gray-300 dark:border-gray-600 text-right">อัตราแปลง</th>
                            ...
                            <tbody>
                                ...
                                <tr class="hover:bg-gray-200 dark:hover:bg-gray-600 border-b border-gray-200 dark:border-gray-700">
                                    <td class="py-2 px-4 text-center">{{ ($units->currentPage() - 1) * $units->perPage() + $loop->iteration }}</td>
                                    <td class="py-2 px-4">{{ $unit->name }}</td>
                                    <td class="py-2 px-4 font-mono text-xs">{{ $unit->code }}</td>
                                    <td class="py-2 px-4">{{ $unit->symbol }}</td>
                                    <td class="py-2 px-4">{{ $unit->company->name ?? '-' }}</td>
                                    <td class="py-2 px-4">{{ $unit->baseUnit->name ?? '-' }}</td>
                                    <td class="py-2 px-4 text-right">{{ $unit->conversion_factor }}</td>
                                    ...
                                    <td class="py-2 px-4 text-center">
                                        <div class="flex justify-center space-x-2">
                                            <a href="{{ route('units.show', $unit) }}" class="text-blue-500 hover:text-blue-700" title="ดูรายละเอียด">
                                                <svg class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z" />
                                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z" />
                                                </svg>
                                            </a>
                                            <a href="{{ route('units.edit', $unit) }}" class="text-yellow-500 hover:text-yellow-700" title="แก้ไข">
                                                <svg class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" />
                                                </svg>
                                            </a>
                                            <form action="{{ route('units.destroy', $unit) }}" method="POST" class="inline" onsubmit="return confirm('ลบหน่วยนับนี้?');">
                                                @csrf @method('DELETE')
                                ...
                                @empty
                                <tr>
                                    <td colspan="10" class="py-4 text-center text-gray-500 dark:text-gray-400">ไม่พบข้อมูลหน่วยนับ</td>
                                </tr>
                                @endforelse
                            </tbody>
                        </table>
