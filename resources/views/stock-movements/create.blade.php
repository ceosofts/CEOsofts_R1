<x-app-layout>
    <x-slot name="header">
        <div class="flex justify-between items-center">
            <h2 class="font-semibold text-xl text-gray-800 leading-tight">
                {{ __('เพิ่มรายการเคลื่อนไหวสินค้า') }}
            </h2>
                    ...
                    <form method="POST" action="{{ route('stock-movements.store') }}">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                            ...
                            <div>
                                ...
                                <x-text-input id="movement_date" type="date" name="movement_date" :value="old('movement_date', date('Y-m-d'))" required class="mt-1 block w-full text-sm" />
                            ...
                            <div>
                                ...
                                <select id="product_id" name="product_id" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50 text-sm">
                                    ...
                                    @foreach ($products as $product)
                                        <option value="{{ $product->id }}" data-stock="{{ $product->stock_quantity }}" data-unit="{{ $product->unit_id }}">{{ $product->code }} - {{ $product->name }} (คงเหลือ: {{ number_format($product->stock_quantity, 2) }})</option>
                                    @endforeach
                                </select>
                            ...
                            <div>
                                ...
                                <x-text-input id="quantity" type="number" step="0.01" min="0.01" name="quantity" :value="old('quantity')" required class="mt-1 block w-full text-sm" />
                            ...
                            <div>
                                ...
                                <x-text-input id="reference_type" type="text" name="reference_type" :value="old('reference_type')" class="mt-1 block w-full text-sm" />
                            ...
                            <div>
                                ...
                                <x-text-input id="reference_id" type="text" name="reference_id" :value="old('reference_id')" class="mt-1 block w-full text-sm" />
                            ...
                            <div class="md:col-span-2">
                                ...
                                <textarea id="note" name="note" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50 text-sm">{{ old('note') }}</textarea>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            // ตรวจสอบจำนวนสินค้าเมื่อเบิกออก และเลือกหน่วยนับอัตโนมัติ
            productSelect.addEventListener('change', function() {
                const selectedOption = this.options[this.selectedIndex];
                if (selectedOption) {
                    // ตั้งค่าหน่วยนับอัตโนมัติ
                    const unitId = selectedOption.dataset.unit;
                    if (unitId) {
                        unitSelect.value = unitId;
                    }
                    
                    // ตรวจสอบเงื่อนไขเมื่อเป็นการเบิกออก
                    if (movementType.value === 'out') {
                        const stockAmount = parseFloat(selectedOption.dataset.stock || 0);
                        quantityInput.setAttribute('max', stockAmount);
                        
                        if (parseFloat(quantityInput.value) > stockAmount) {
                            quantityInput.value = stockAmount;
                        }
                    } else {
            ...
            // เมื่อเลือกประเภทเป็น "เบิกออก" ให้ตรวจสอบจำนวนสินค้า
            movementType.addEventListener('change', function() {
                if (this.value === 'out' && productSelect.value) {
